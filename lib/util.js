...
/**
 * makes collection name from a class defination
 *
 ...
 * @public
 */
util.makeCollectionName = function (Model) {
...
/**
 * makes the pivotCollection name using two different models and
 ...
 * @public
 */
util.makePivotCollectionName = function (Model, RelatedModel) {
  let modelName = Model.name
...
util.parseNestedRelations = function (relations) {
  ...
  return {
    root: relationsKeys[0],
    nested: (relationsKeys && relationsKeys.length) ? _.tail(relationsKeys).join('.') : null
  }
}
